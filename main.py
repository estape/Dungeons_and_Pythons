...
def perguntar_ia(historico, mensagem_jogador):
    ...
    historico.append({"role": "user", "content": mensagem_jogador})
    ...
    try:
        resposta = requests.post(API_URL, headers=headers, data=json.dumps(data))
        ...
        if resposta.status_code == 200 and "choices" in resposta_json and resposta_json["choices"]:
            ...
            return f"Erro na API: {resposta_json.get('message', resposta_json)}"
